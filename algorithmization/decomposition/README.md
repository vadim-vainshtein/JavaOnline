...
В алгоритме используются методы класса Util getDigits() и sum() для определения суммы цифр числа.
...
-----------------
	...
	algorithmization.decomposition.task15
	
*Найти все натуральные n-значные числа, цифры которых образуют строго возрастающую последовательность*

Используется класс

	algorithmization.decomposition.Numbers

Класс Numbers содержит массив, который заполняется числами по одному из правил. В данном случае для заполнения массива числами из возрастающих последовательностей цифр используется метод

	Numbers.initDigitsAscending();

Этот метод вызывает рекурсивную функцию, заполняющую поочерёдно все разряды, и записывающую получающиеся числа в массив. Число получается из массива цифр при помощи метода

	algorithmization.Util.buildNumberFromDigits()


Задача 16
-----------------
	
	algorithmization.decomposition.task16
	
*Определить сумму n-значных чисел, содержащих только нечётные цифры. Определить также, сколько чётных цифр в найденной сумме.*

Как и в предыдущей задаче, используется класс Numbers. Нужные числа получаются при помощи метода 

	Numbers.initDigitsOdd(),

устроенного таким же образом, как и initDigitsAscending(), но с другим правилом отбора цифр. Суммирование и нахождение количества чётных цифр осуществляется в методах

	Util.sum()
	Util.countEvenDigits()

Последний использует метод

	Util.getDigits()

для разбора числа по цифрам.


Задача 17
-----------------

	algorithmization.decomposition.task17
	
*Из заданного числа вычли сумму его цифр. Из результата снова вычли сумму цифр, и т.д. Сколько таких действий надо произвести, чтобы получить 0?*

Используем методы класса algorithmization.Util

	Util.getDigits();
	Util.sum();

В классе Main составим отдельный метод countSubtractions() для вычисления количества необходимых итераций.
